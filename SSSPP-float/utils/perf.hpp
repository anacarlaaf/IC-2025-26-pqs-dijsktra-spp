...
struct CacheStats {
    ...
    PerfCounter instructions  { PERF_TYPE_HARDWARE, PERF_COUNT_HW_INSTRUCTIONS }; // <─ novo

    uint64_t r_l1, r_l2, r_llc, r_cycles, r_instructions;
    ...
    void start() {
        ...
        llc_load_miss.start();
        cycles.start();
        instructions.start();   // <─ novo
    }

    void stop() {
        r_instructions = instructions.stop();   // <─ novo
    ...
    void print(const char* label) {
        double ipc = (r_cycles > 0) ? (double)r_instructions / r_cycles : 0.0;
