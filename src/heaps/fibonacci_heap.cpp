...
class fibonacci_heap
{
  public:
        ...
        : min_root(NULL), nodes(NULL), num_nodes(0), num_trees(0), max_num_nodes(capacity), A(NULL), Dn(0)
    {
        ...
        Dn = 1 + (int)(log(keyType(max_num_nodes)) / log(2.));
    ...
    par extract_min()//ll& ret_key)
    {
        ...
        min_node = min_root;
        if (min_node == NULL)
            return make_pair(-1, 0); // heap is empty now
        ...
        result = {min_node->key, min_node->index};
        //ret_key = min_node->key;
        clear_node(result.second);
     ...
     */
    int get_key(int index, keyType& ret_key)
    {
